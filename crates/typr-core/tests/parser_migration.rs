...
fn verify_parser_parity(code: &str, file_name: &str) {
    ...
    let (remaining, old_ast) =
        parse_elements(span).expect("Old parser failed to parse the expression");
    assert!(
        remaining.fragment().trim().is_empty(),
        "Old parser did not consume the entire input! Remaining: {}",
        remaining.fragment()
    );
...
migration_test!(test_expr_paren, "parser_migration_cases/expr_paren.ty");
